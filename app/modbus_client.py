...
class MPPTData:
    ...
    modules: list[MPPTModuleData] | None = None  # All available modules with diagnostics
...
class OperatingStateFormatter:
    ...
    @classmethod
    def format_state(cls, state_value: int | None) -> str:
...
class ModuleEventsDecoder:
    ...
    @classmethod
    def decode_events(cls, events_bitfield: int | None) -> str:
...
class DiagnosticData:
    ...
    temperature: float | None  # Celsius, None if unavailable
    operating_state: int | None  # Enum value, None if unavailable
    module_events: int | None  # Bitfield, None if unavailable
    ...
    @classmethod
    def create(
        cls, temperature: float | None, operating_state: int | None, module_events: int | None
    ) -> DiagnosticData:
...
class ModbusClient:
    ...
    def read_device_info(self) -> dict[str, str] | None:
        """
    ...
    def read_mppt_data(self) -> MPPTData | None:
        """
        ...
        try:
            ...
            for i in range(num_modules):
                try:
                    # Read power data
                    voltage = (
                        float(model_160.module[i].DCV.cvalue) if model_160.module[i].DCV.cvalue is not None else 0.0
                    )
                    current = (
                        float(model_160.module[i].DCA.cvalue) if model_160.module[i].DCA.cvalue is not None else 0.0
                    )
                    ...
                    diagnostic_data = DiagnosticData.create(
                        temperature=temperature, operating_state=operating_state, module_events=module_events
                    )

                    # Create MPPT channel data for backward compatibility
                    mppt_channel = MPPTChannelData(voltage=voltage, current=current, power=power)
                    mppt_channels.append(mppt_channel)

                    # Create module data with diagnostics
                    module_data = MPPTModuleData(
                        voltage=voltage, current=current, power=power, diagnostics=diagnostic_data
                    )
                    ...
                    logger.debug(
                        f"MPPT{i + 1}: {voltage}V, {current}A, {power}W, "
                    ...
                    modules_with_diagnostics.append(
                        MPPTModuleData(voltage=0.0, current=0.0, power=0.0, diagnostics=empty_diagnostics)
                    )
