...
from app.modbus_client import DiagnosticData, ModbusClient
...
class TestDiagnosticGracefulDegradation:
    ...
    def test_diagnostic_mqtt_failure_doesnt_affect_core_sensors(self, mock_mqtt_client):
        ...
        def mock_publish(topic, payload, qos, retain):
            ...
            if (
                "diagnostic" in topic
                or "temperature" in topic
                or "operating_state" in topic
                or "module_events" in topic
            ):
        ...
        diagnostic_data = [DiagnosticData.create(45.2, 4, 0), DiagnosticData.create(None, 2, 3)]
